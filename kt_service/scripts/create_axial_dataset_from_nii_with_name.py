...
dicom_dataset_dir = '/media/msi/fsi9/fsi/datasets_mrt/svalka/COVID19_1110_neponytnye_failes/convert/all/'
...
def sc_cnt(coord_str, pix_spc):
    """
    Преобразует контур из пикселей в мм (относительно верхнего левого угла).

    Parameters:
    - coord_str: array of shape (N, 1, 2) or (N, 2), like from cv2.findContours
    - pix_spc: tuple (dx, dy, dz) — voxel sizes in mm

    Returns:
    - строка с координатами X Y X Y ... в мм
    """
    # Приводим к форме (N, 2)
    if coord_str.ndim == 3 and coord_str.shape[1] == 1:
        coords = coord_str[:, 0, :]  # (N, 1, 2) -> (N, 2)
    else:
        coords = coord_str.copy()

    # Проверяем замкнутость (сравниваем первую и последнюю точку)
    first = coords[0]
    last = coords[-1]
    if not (first == last).all():
        coords = np.vstack([coords, first])  # добавляем первую точку

    # Переводим в мм: умножаем координаты на размер пикселя
    # ВАЖНО: coords[i] = [col, row] → [x, y]
    # pixel_spacing[0] = ширина по X (столбцы), [1] = высота по Y (строки)
    xs = coords[:, 0] * pix_spc[0]  # j → x
    ys = coords[:, 1] * pix_spc[1]  # i → y

    # Собираем пары X Y
    end_list = []
    for x, y in zip(xs, ys):
        end_list.append(float(x))
        end_list.append(float(y))
...
def create_femm_mask_file(mask_list, color_output, img_normalized, final_output, file_name, pix_spc, only_body_mask):
    """
    Создаёт YOLO-аннотации для масок тканей.
    """
    img_normalized_clear = img_normalized.copy()
    classes_list = {'bone': '0', 'muscles': '1', 'lung': '2', 'adipose': '3', 'only_body_mask': '4'}

    path_to_save_labels = f'../../save_test_masks/from_nii/{file_name}/'
    check_work_folders(os.path.join(os.getcwd(), path_to_save_labels))
    file_path = f'{path_to_save_labels}/{file_name}.txt'

    # Убедимся, что файл пуст в начале (если нужно дописывать — уберите эту строку)
    ...
    def ensure_closed_contour(coords):
        ...
        if not np.array_equal(first, last):
            coords = np.vstack([coords, first])
        return coords

    # Обработка обычных масок
    for msk in mask_list:
        class_msk = classes_list[msk[-1]]

        # Преобразуем маску в бинарную
        if msk[0].dtype != np.uint8:
            msk_binary = (msk[0] > 0).astype(np.uint8) * 255
        else:
            msk_binary = msk[0]

        # Разбиваем на отдельные связные компоненты
        num_labels, labels = cv2.connectedComponents(msk_binary)
        all_contours = []

        for i in range(1, num_labels):  # пропускаем фон (метка 0)
            comp_mask = (labels == i).astype(np.uint8) * 255
            contours, _ = cv2.findContours(comp_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
            all_contours.extend(contours)

        # Фильтруем маленькие шумы
        min_area = 4
        msk_contours = [cnt for cnt in all_contours if cv2.contourArea(cnt) > min_area]

        for cnt in msk_contours:
            if len(cnt) < 5:
                continue

            # Подготавливаем контур для YOLO
            scaled_contour = sc_cnt(cnt, pix_spc)

            with open(file_path, "a") as file:
                file.write(f'{class_msk} {scaled_contour}\n')

    # === Обработка only_body_mask как класса '4' ===
    if only_body_mask is not None and only_body_mask.any():
        if only_body_mask.dtype != np.uint8:
            body_binary = (only_body_mask > 0).astype(np.uint8) * 255
        else:
        ...
        for cnt in body_contours:
            ...
            coords = cnt.reshape(-1, 2).astype(np.float64)

            # Гарантируем замкнутость
            coords = ensure_closed_contour(coords)

            # Преобразуем в формат для записи (предполагается, что sc_cnt принимает контур в формате OpenCV)
            # Но sc_cnt, судя по названию, ожидает контур в формате OpenCV (Nx1x2), поэтому восстановим форму
            closed_cnt = coords[:-1].reshape(-1, 1, 2)  # убираем дублирующую последнюю точку при передаче в sc_cnt?
            # Однако если sc_cnt работает с (N,2), передайте coords напрямую.
            # Уточните сигнатуру sc_cnt. Ниже предполагается, что она принимает (N,2) или OpenCV-контур.

            # ВАЖНО: если sc_cnt ожидает OpenCV-формат (N,1,2), то:
            if coords.shape[0] > 0:
                opencv_contour = coords.reshape(-1, 1, 2).astype(np.int32)
                scaled_contour = sc_cnt(opencv_contour, pix_spc)

                with open(file_path, "a") as file:
                    file.write(f'4 {scaled_contour}\n')
    ...
    for nii_arch in tqdm(nii_archive_name):
        ...
        for hu_img_orig in slise_save_list:
            ...
            create_femm_mask_file([mask_muscles, mask_bone, mask_lung, mask_adipose], color_output, img_normalized,
                                  final_output, file_name, img.header.get_zooms(), only_body_mask)
            cnt += 1
